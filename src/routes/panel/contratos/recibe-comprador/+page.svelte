<script lang="ts">
  ...
  import { CONTRACT_NAMES } from '@/modules/contracts/constants/contracts'
  import type { IFolderStructure } from '@/modules/properties/interfaces/IProperty'
  ...
  let pactos: IPacto[] = $state([])
  onMount(async () => {
    pactos = (await api.get(`/api/private/pactos/${CONTRACT_NAMES.RESERVA_COMPRADOR}`)).data
  })

  let name: string = $state('')
  let dni: string = $state('')
  let address: string = $state('')
  let booking: number = $state(0)
  ...
  let arrasDate: Date = $state(new Date())
  let priceDate: Date = $state(new Date())
  let signatureDate: Date = $state(new Date())
...
<div class="contract-recibe-comprador g-contract">
  <div class="inputs">
    ...
    <div class="comprador-inputs">
      ...
      <Input bind:value={dni} label="DNI o NIE" error={(dni && !checkDNI(dni)) || false} errorMessage={'¡El DNI o NIE no es correcto!'}
      ></Input>
