...
export const orderTipoOfers = (tipo_ofer: string | null) => {
...
export const filterProperties = (properties: IProperty[], filters: IFilters): IProperty[] => {
  ...
  newProperties = newProperties.map((property) => {
    ...
    if (filters.tipoOfer.length && !filters.tipoOfer.includes(updatedProperty.tipo_ofer)) updatedProperty.hide = true
    ...
    if (updatedProperty.accion === 'Vender') {
      if (filters.priceMax && updatedProperty.precioCompra > toNumber(filters.priceMax)) updatedProperty.hide = true
      if (filters.priceMin && updatedProperty.precioCompra < toNumber(filters.priceMin)) updatedProperty.hide = true
    } else {
      if (filters.priceMax && updatedProperty.precioAlquiler > toNumber(filters.priceMax)) updatedProperty.hide = true
      if (filters.priceMin && updatedProperty.precioAlquiler < toNumber(filters.priceMin)) updatedProperty.hide = true
    }

    if (filters.sizeMax && updatedProperty.metrosCons > toNumber(filters.sizeMax)) updatedProperty.hide = true
    if (filters.sizeMin && updatedProperty.metrosCons < toNumber(filters.sizeMin)) updatedProperty.hide = true

    if (filters.rooms && updatedProperty.habitaciones < toNumber(filters.rooms)) updatedProperty.hide = true
    if (filters.baths && updatedProperty.banyos < toNumber(filters.baths)) updatedProperty.hide = true
  ...
  const sortingFunctions: Record<string, (a: IProperty, b: IProperty) => number> = {
    'Más baratos': (a, b) => a.precioCompra - b.precioCompra,
    'Más caros': (a, b) => b.precioCompra - a.precioCompra,
    'Más pequeños': (a, b) => a.metrosCons - b.metrosCons,
    'Más grandes': (a, b) => b.metrosCons - a.metrosCons,
  }
...
export const getFilters = (properties: IProperty[]) => {
  ...
  const ofers = [...new Set(properties.map((property) => property.tipo_ofer))]
  ...
  const groupedByCity = locations.reduce((acc, item) => {
