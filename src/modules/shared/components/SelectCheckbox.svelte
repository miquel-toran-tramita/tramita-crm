<script lang="ts">
  ...
  const clean = () => {
    options = options.map((opt: ILabelValue) => {
      opt.checked = false
      return opt
    })
  ...
  <div class="dropdown" class:isOpen bind:this={HTMLDropdown}>
    <div class="options" data-lenis-prevent>
      {#each options as option}
        <button class="option" onclick={() => (option.checked = !option.checked)}>
          <Checkbox bind:value={option.checked} />
          {@html option.label}
        </button>
      {/each}
      {#if !hideDelete}
