<script lang="ts">
  ...
  let { contacts = [], agents = [], creator, events = $bindable([]) }: Props = $props()
...
<div class="timeframe-container">
  ...
  {#if timeFrame === 'week'}
    <CalendarWeek {events} bind:globalDate {agents} {creator} {contacts} {organizeBy} {updateInputDate} {agentsOptions} />
  {/if}

  {#if timeFrame === 'week5'}
    <CalendarWeek {events} bind:globalDate {agents} {creator} {contacts} {agentsOptions} {updateInputDate} simplified />
  {/if}

  {#if timeFrame === 'month'}
    <CalendarMonth {events} bind:globalDate {agents} {creator} {contacts} {agentsOptions} {updateInputDate} />
  {/if}
...
<div class="g-new-btn-container">
  <CreateEvent bind:events {contacts} {creator} {agentsOptions} />
</div>
